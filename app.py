...
configuration = Configuration(access_token=CHANNEL_ACCESS_TOKEN)  #輸入你的access token
handler = WebhookHandler(CHANNEL_SECRET) # 輸入你的 channel secret
...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        bot_msg = menu
        # 如果 user_msg 存在於 faq 字典內
        if user_msg in faq:
            # 將回覆訊息改為 faq 字典內對應的訊息
            bot_msg = faq[user_msg]
        elif user_msg in table:
            buy = table[user_msg]["buy"]
            sell = table[user_msg]["sell"]
            bot_msg = TextMessage(text=f"{user_msg} 買價:{buy} 賣價:{sell} 資料取自於臺灣銀行公告牌價")
